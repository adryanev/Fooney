//
//  QuoteMessage.swift
//  Fooney
//
//  Created by Adryan Eka Vandra on 23/03/23.
//

public class QuoteMessage {
    private init() {}

   public static var happy = [
        "Because of your smile, you make academy more beautiful.",
        "Share your smile with the academy. It’s a symbol of friendship and peace.",
        "A smile is a curve that sets everything straight.",
        "Always wear a smile sometime during the academy, it makes you feel happier and younger.",
        "The purpose of our lives is to be happy. Spread happiness to all of the fellows learners!"
   ]
   public static var angry = [
        "For every minute you are angry you lose sixty seconds of happiness.",
        "Don’t waste your time in anger, regrets, worries and grudges. Life is too short to be unhappy.",
        "Never go to bed mad. Stay up and fight.",
        "Anger solves nothing it only puts up your blood pressure.",
        "The best way to deal with anger is to prevent it."
   ]
   public static var disgust = [
        "Don't feel disgust. Believe you can and you're halfway there.",
        "It's okay to feel disgusted. Your emotions are valid and important, and they serve a purpose.",
        "You're not alone in feeling disgusted. Everyone experiences this emotion at some point in their life.",
        "Allow yourself to feel disgusted, but don't let it consume you. Use it as a tool for learning and growth.",
        "Don't feel disgusted with yourself. Because that's one of the causes of your insecurities."
   ]
   public static var fear = [
        "The only thing we have to fear is fear itself. Don't be afraid and just let it flow.",
        "Fear is not real. The only place that fear can exist is in our thoughts of the future. It is a product of our imagination.",
        "Expose yourself to your deepest fear; after that, fear has no power, and the fear of freedom shrinks and vanishes. You are free.",
        "Take every chance and drop every fear you have.",
        "The fears we don't face become our limits"
   ]
    public static var neutral = [
        "Just for today, smile a little more.",
        "Make yourself proud of what you already did. Spread a positive impacts to all of the fellow academic learners.",
        "Smile more. Worry less. Be thankful.",
        "You don't have to be the best. Just keep doing your best.",
        "Always keep smiling!"
    ]
    public static var sads = [
        "Dear me, I know you're scared, but you can handle this.",
        "One day you'll tell your story of how you overcame what you went through and it will be someone else's survival guide.",
        "Don't be sad. You are gonna do great today!",
        "Don't be so hard on yourself, okay? You're doing a really good job!",
        "Every bad situation will have something positive, make sure to rest and youll feel okay again soon."
    ]

   public static var surprise = [
        "Life is full of surprises! What;s coming is better than what is gone.",
        "Surprise your doubts with action!",
        "A little progress each day adds up to big results!",
        "Be proud of how far you've come and never stop pushing to be the best you can be.",
        "When you least expect it, life might SURPRISE you. It might turn out even better than you'd hoped."
   ]
}
